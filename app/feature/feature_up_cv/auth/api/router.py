...
# Feedback Agent router (optional). Imported with try/except so that
# branches without the feedback_agent module (e.g. rebuild_feature_upCV)
# still register the core endpoints cleanly.
try:
    from app.feature.feature_up_cv.auth.api.endpoints.score_feedback import (
        router as score_feedback_router,
    )
    _feedback_router_available = True
except ImportError:
    _feedback_router_available = False
